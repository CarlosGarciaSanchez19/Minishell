...
/*   Updated: 2025/03/25 19:00:53 by carlosg2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	command_child_exec(t_tokens *curr_tkn, t_pipes *p, t_shell *shell)
{
	...
	if (curr_tkn->cmd && !is_built_in(curr_tkn))
	{
		...
		i = 0;
		while (curr_tkn->cmd_args && curr_tkn->cmd_args[i])
		{
			if (is_emptyquote(curr_tkn->cmd_args[i]))
			{
				command_array[i + 1] = "";
				curr_tkn->cmd_args[i] = "";
			}
	...
	else if (curr_tkn->cmd && is_built_in(curr_tkn) &&
			(curr_tkn->special || p->num_pipes))
...
pid_t	fork_redir_and_exec(t_tokens *curr, t_shell *shell, t_pipes *p, int i)
{
	...
	else if (pid > 0)
	{
		...
		if (curr->cmd && is_built_in(curr) &&
			(!curr->special && !p->num_pipes))
